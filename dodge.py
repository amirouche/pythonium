...
def dodge(filepath, output=None):
    ...
    javascript = translator.writer.value()
...
def main(argv=None):
    ...
    Usage: pythonium [-h][-v] [FILE ...] [-o FILE]
    ...
    args = docopt(doc, argv, version=VERSION)
    filepaths = args['FILE']
    if not filepaths:
        main(['--help'])
        sys.exit(-1)
    ...
    if outfile:
        with open(outfile, 'w') as output:
            for filepath in filepaths:
                generate_js(filepath, output)
    else:
        for filepath in filepaths:
            generate_js(filepath, sys.stdout)
